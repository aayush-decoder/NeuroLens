...
POST /adapt{text, language, level}Returns annotated HTML paragraphLets you cache adapted paragraphs by content hash — same paragraph on two devices never calls Gemini twice

POST /review{paragraphs[]}Returns [{term, definition}]Same caching benefit; also lets you persist review sheets to a user account later

POST /session{url, scrollY, adaptedMap, startTime}Saves session state server-sideEnables cross-device session restore (right now you're locked to one machine via chrome.storage.local)


The caching on /adapt is the single highest-value addition — a content hash of the paragraph as the cache key means popular articles (Medium, Wikipedia) get adapted once and served instantly to everyone after. All three routes can be ~30 lines of Python/Node each.

What you don't need a backend for (keep client-side): telemetry tracking, eye-strain timing, scroll persistence for single-device use, file loading.
